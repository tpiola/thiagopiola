    ...
    >
      ...
      <div className="relative mx-auto max-w-6xl px-5 md:px-8">
        <Reveal className="max-w-3xl" variant="left">
          ...
          <p className="mt-5 text-base leading-relaxed text-muted max-w-2xl">{tecnologia.lead}</p>
        </Reveal>

        <Stagger className="mt-14 overflow-hidden border border-border bg-surface rounded-xl" stagger={0.06}>
          {tecnologia.items.map((item, i) => (
            <StaggerItem key={item.title}>
              <motion.div
                className="group relative flex flex-col gap-1.5 border-b border-border px-6 py-5 last:border-b-0 transition-all duration-300 hover:bg-[color-mix(in_srgb,var(--brand)_5%,var(--surface))] sm:flex-row sm:items-center sm:justify-between md:px-8 md:py-6"
              ...
              >
                ...
                <div className="flex items-center gap-5">
                  <span className="font-mono text-[10px] font-bold tracking-[0.2em] text-muted group-hover:text-[var(--brand)] transition-colors duration-300 flex-shrink-0">
                ...
                <div className="flex items-center gap-4 sm:max-w-md sm:text-right justify-start sm:justify-end">
                  ...
                  <motion.span className="hidden sm:block flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 text-[var(--brand)]">
                    →
                  </motion.span>
                </div>
              </motion.div>
